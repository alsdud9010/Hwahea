...
<div class="right_col" role="main">
	<div class="">
		<div class="page-title">
			...
			<form action="adminIngredientList.do" id="adminIngredient_form" method="get">
			</form>
				<div class="x_panel">
					...
					<table class="table">
						...
						<c:forEach var="ingredient" items="${list }">
							<tr>
								<td>${ingredient.i_KName }(${ingredient.i_EName })</td>
								<td>${ingredient.i_prop }</td>
								<td>${ingredient.i_grade }</td>
								<td>${ingredient.i_datagrade }</td>
								<td>
									<c:if test="${ingredient.i_warning != null }">
										해당
									</c:if>
									<c:if test="${ingredient.i_warning == null }">
										해당 사항 없음
								...
								<td>
									<c:if test="${ingredient.i_oilly == 'y' }">
										해당
									</c:if>
									<c:if test="${ingredient.i_oilly == 'n' }">
										도움 되지 않음
									</c:if>
									<c:if test="${ingredient.i_oilly == null }">
										해당 사항 없음
									</c:if>
								</td>
								<td>
									<c:if test="${ingredient.i_dry == null }">
										해당 사항 없음
									</c:if>
									<c:if test="${ingredient.i_dry == 'y' }">
										해당
									</c:if>
									<c:if test="${ingredient.i_dry == 'n' }">
										도움 되지 않음
									</c:if>
								</td>
								<td>
									<c:if test="${ingredient.i_sensitive == null }">
										해당 사항 없음
									</c:if>
									<c:if test="${ingredient.i_sensitive == 'y' }">
										해당
									</c:if>
									<c:if test="${ingredient.i_sensitive == 'n' }">
										도움 되지 않음
									</c:if>
								</td>
								<td>
									<c:if test="${ingredient.i_func != null }">
										해당
									</c:if>
									<c:if test="${ingredient.i_func == null }">
										해당 사항 없음
								...
								<td>
									<c:if test="${ingredient.i_allergie != null }">
										해당
									</c:if>
									<c:if test="${ingredient.i_allergie == null }">
										해당 사항 없음
			...
			<div style="text-align:center;">
						<button type="button" class="btn btn-success btn-lg" data-toggle="modal"
									data-target=".bs-example-modal-lg">성분 등록</button>
						<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
							<div class="modal-dialog modal-lg">
										<div class="modal-content">
											...
											<form:form commandName="ingredientcommand" action="adminIngredientRegister.do"
											...
											<form:input type="text" path="i_datagrade" class="form-control"/>
											
											<label class="control-label" for="i_warning">20가지 주의성분 해당 여부</label>
											<form:select type="text" path="i_warning" class="form-control">
												<form:option value="">해당 사항 없음</form:option>
												<form:option value="y">해당 됨</form:option>
											</form:select>
											
											<label class="control-label" for="i_oilly">지성 피부 도움 여부</label>
											...
											<form:select type="text" path="i_sensitive" class="form-control">
												<form:option value="">해당 사항 없음</form:option>
												<form:option value="y">해당 됨</form:option>
												<form:option value="n">도움 되지 않음</form:option>
											</form:select>
											...
											<form:select type="text" path="i_allergie" class="form-control">
												<form:option value="">해당 사항 없음</form:option>
												<form:option value="y">해당 됨</form:option>
											</form:select>
												</div>
